...
// ... existing code ...
     ...
     * @param callback 回调
     */
    public void getPostsByUserId(int userId, int page, int size, RepositoryCallback<List<Post>> callback) {
        new Thread(() -> {
            String response = networkManager.getPostsByUserId(userId, page, size);
            ...
            try {
                ...
                if (jsonResponse.getInt("code") == 0) {
                    ...
                    String errorMsg = jsonResponse.optString("msg", "获取用户帖子失败");
                    if (callback != null) callback.onError(errorMsg);
                }
            } catch (Exception e) {
                e.printStackTrace();
                if (callback != null) callback.onError("解析用户帖子失败：" + e.getMessage());
            }
        }).start();
    }
